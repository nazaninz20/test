...
% for i=1:17
%     if eval(['(size(stage_' num2str(i) ',1)>0)'])
%         eval(['threshold(i) = findHistThreshold(stage_' num2str(i) ');']);
%     end
% end
...
load ('test_sequence_labels');
% l = accumulative_reward(:,3);
l = test_sequence_labels;
accumulative_reward(:,3) = l;
...
% got 68% for 474 remove the last stage (thr Thr=474 and 475 works best on test and train data)
